...
const ANIME_CATEGORIES = [
  {
    ...
    kind: 'anime',
    sort: 10,
    match: ['1930', '1940', 'senkousha', 'kenzo', 'ofuji'],
  },
  {
    slug: 'anime-shorts',
    name: 'Shorts',
...
function isUsableUrl(url) {
  return (
    typeof url === 'string' && !/\s/.test(url) && /^https:\/\/\S+$/i.test(url)
  );
}
...
for (const entry of enriched) {
  ...
  const title = String(
    toArray(meta.metadata?.title ?? doc.title)[0] ?? '',
  ).trim();
  if (!title) continue;
  ...
  const streamUrl = `${DOWNLOAD}/${encodeURIComponent(
    doc.identifier,
  )}/${video.name.split('/').map(encodeURIComponent).join('/')}`;
  if (!isUsableUrl(streamUrl)) continue;

  const yearRaw = Number(toArray(meta.metadata?.year ?? doc.year)[0]);
  const dateYear = Number(
    String(toArray(meta.metadata?.date ?? doc.date)[0] ?? '').slice(0, 4),
  );
  const year =
    [yearRaw, dateYear].find(
      y => Number.isInteger(y) && y >= 1888 && y <= 2100,
    ) ?? null;
  ...
  const description =
    String(toArray(meta.metadata?.description ?? doc.description)[0] ?? '')
  ...
  const tags = [
    ...toArray(meta.metadata?.subject ?? doc.subject),
    ...toArray(meta.metadata?.collection),
  ]
    .map(String)
    .flatMap(s => s.split(/[;,]/));
  ...
  rows.push({
    ...
    title,
    description,
    poster_url: `https://archive.org/services/img/${encodeURIComponent(
      doc.identifier,
    )}`,
    ...
    license: String(
      toArray(meta.metadata?.licenseurl ?? doc.licenseurl)[0] ?? '',
    ),
  });
}

console.log(
  `${rows.length} importable ${
    KIND_PLURAL[options.kind]
  } (license-filtered, deduplicated)`,
);

let final = rows;
if (options.probe) {
  console.log(
    `probing ${final.length} files with ${options.concurrency} in flight...`,
  );
  const verdicts = await mapPool(final, options.concurrency, r =>
    isPlayable(r.stream_url),
  );
...
const usedCategories = CATEGORIES.filter(rule =>
  final.some(r => r.category.slug === rule.slug),
);

const sql = `-- Generated by scripts/import-archive.mjs on ${new Date()
  .toISOString()
  .slice(0, 10)}
-- Source: https://archive.org (hosts the files it indexes)
-- Filters: kind=${options.kind} collections=${options.collections.join(
  ',',
...
console.log(`\nwrote ${options.out}`);
console.log(
  `  ${final.length} ${KIND_PLURAL[options.kind]}, ${
