...
const options = {
  ...
  // Upstream categories that ignore `countries`/`languages` entirely.
  //
  // Without this the geo filter is all-or-nothing, and that is the wrong shape
  // for what people actually want: local news and local general channels, but
  // sport from wherever it happens to be broadcast. A BD/ben run matches
  // exactly three sports channels -- almost nobody broadcasts sport in
  // Bengali, so the POOL is the limit, not the importer. `--everywhere=sports`
  // lifts that one category out of the filter and leaves the rest untouched.
...
const CATEGORY_RULES = [
  {
    ...
    name: 'News',
    sort: 10,
    match: ['news', 'weather', 'legislative'],
  },
  {
    slug: 'sports-tv',
    name: 'Sports Channels',
    sort: 30,
    // 'outdoor' is deliberately NOT matched here. Upstream uses it for
    // hunting, fishing and farming: of the 49 channels carrying it and nothing
    // else, most are livestock and agriculture. Matching it put Agro TV,
    // DD Kisan and a dog-show channel on the Sports rail, which is why that
    // rail looked wrong even before it looked empty. It sits under Lifestyle.
    match: ['sports'],
  },
  {
    slug: 'kids-tv',
  ...
  { slug: 'religious-tv', name: 'Religious', sort: 70, match: ['religious'] },
  {
    ...
    name: 'Knowledge',
    sort: 80,
    match: ['documentary', 'education', 'science', 'business'],
  },
  {
    slug: 'entertainment',
    name: 'Entertainment',
    sort: 20,
    match: ['entertainment', 'series', 'comedy', 'family', 'culture'],
  },
  {
    slug: 'lifestyle-tv',
    name: 'Lifestyle',
    sort: 90,
    match: ['lifestyle', 'cooking', 'travel', 'relax', 'auto', 'outdoor'],
  },
  {
    slug: 'general-tv',
    name: 'General',
    sort: 100,
    match: ['general', 'public', 'interactive', 'shop'],
  },
];
...
for (const logo of logos) {
  ...
  if (!current || (logo.width ?? 0) > (current.width ?? 0))
    logoById.set(logo.channel, logo);
}
...
for (const stream of streams) {
  ...
  // Checked alongside the geo tests rather than after them, so a category
  // named in `--everywhere` is admitted on its own merit. `--categories` still
  // applies to it below: widening where a category may come FROM does not
  // override which categories the run is for.
  const isEverywhere = channel.categories.some(c =>
    options.everywhere.includes(c),
  );
  if (!matchesCountry && !matchesLanguage && !isEverywhere) continue;

  if (
    options.categories &&
    !channel.categories.some(c => options.categories.includes(c))
  ) {
    continue;
  }
  ...
  const siteHost = channel.website
    ? new URL(channel.website).hostname.replace(/^www\./, '')
    : null;
  const registrable = h => h?.split('.').slice(-2).join('.');
  const isOfficial =
    Boolean(siteHost) && registrable(streamHost) === registrable(siteHost);
...
console.log(
  `matched ${wanted.length} channels (blocklist + nsfw already removed)`,
);

let rows = wanted;
if (options.probe) {
  console.log(
    `probing ${rows.length} streams with ${options.concurrency} in flight...`,
  );
...
rows.sort(
  (a, b) => a.category.sort - b.category.sort || a.name.localeCompare(b.name),
);

const usedCategories = CATEGORY_RULES.filter(rule =>
  rows.some(r => r.category.slug === rule.slug),
);

const sql = `-- Generated by scripts/import-iptv.mjs on ${new Date()
  .toISOString()
  .slice(0, 10)}
-- Source: https://iptv-org.github.io/api/ (index of publicly reachable streams)
-- Filters: countries=${options.countries.join(',') || '-'} languages=${
  options.languages.join(',') || '-'
}${
  options.everywhere.length ? ` everywhere=${options.everywhere.join(',')}` : ''
...
${usedCategories
  .map(c => `  (${sqlText(c.slug)}, ${sqlText(c.name)}, 'live_tv', ${c.sort})`)
  .join(',\n')}
...
${rows
  .map(
    (r, i) =>
      `  (${sqlText(r.slug)}, ${sqlText(r.name)}, ${sqlText(
        r.description,
      )}, ${sqlText(r.logo_url)},\n` +
...
console.log(
  `  ${official} served from the broadcaster's own domain, ${
    rows.length - official
